 ...
 */
@Component
public class SMSApplicationListener implements ApplicationListener {
    private static Logger logger = LoggerFactory.getLogger(SMSApplicationListener.class);
    ...
    public void onApplicationEvent(ApplicationEvent applicationEvent) {
        if (applicationEvent instanceof CommonApplicationEvent) {
            ...
            try {
                logger.info("SMS应用推送-接收到的消息：" + commonApplicationEvent.toString());
                // TODO: 2018/8/1 实现具体业务
            } catch (Exception e) {
                logger.error("SMS应用推送-消息转发失败。失败原因======>{}", e.getMessage());
            }
        }
        if (applicationEvent instanceof RetryApplicationEvent) {
            ...
            logger.info("SMS重试推送应用推送-接收到的消息：" + retryApplicationEvent.toString());
            try {
                logger.info("SMS重试推送应用推送-接收到的消息：" + retryApplicationEvent.toString());
                // TODO: 2018/8/1 实现具体业务
            } catch (Exception e) {
                logger.error("SMS重试推送-消息推送失败。失败原因======>{}", e.getMessage());
            }
        }
    }
}
